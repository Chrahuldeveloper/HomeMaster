const data4 = [
  {
    img: "https://homemaster.in/upload/subservices/1bhk.jpg",
    tittle: "1 BHK",
    price: "Rs 4500 ",
    Introduction:
      "Clean House is pure bliss. In today’s busy world, it is difficult for everyone to take time for deep cleaning of their house. But when your cleaning partners are here, you don’t have to worry. We have expertise in providing deep cleaning to your house. We have trained, efficient, and trusted employees who understand the need of our customers and work accordingly. They are proficient in cleaning and scrubbing the floor in order to remove dirt from pores. They left no scope for stains and work their level best to restore the shine of the floor of your house.",
    Time: "We require two to six hours for cleaning your complete house. The total time will be depended on the type of floor and size of your house.Every time we clean, we'll provide services that include the following:There are a large number of services that we provide every time when you choose our services: ? We will remove cobwebs from your house. ? Thorough dusting of your house and furniture. ? Cleaning of floors. ? Cleaning of toilets. ? Cleaning of mirrors or chrome fixtures. ? Cleaning of Shower doors. ? Cleaning of tile walls.",
    InclusionsExclusions: [
      "Thorough dusting of the apartment, including bathrooms and kitchen",
      "Mopping and scrubbing of the floor area with floor cleaner",
      "Cleaning of other areas of the rooms",
      "Any specialized cleaning tasks not mentioned",
      "Cleaning of exterior areas (e.g., balconies, terraces)",
      "Deep cleaning of upholstery or carpets",
      "Window cleaning (unless specified)",
    ],
    Procedure: [
      "Detailed cleaning of your house consisting of two deep cleaning processes",
      "First step: Thorough dusting of the apartment with special attention to bathrooms and kitchen",
      "Second step: Mopping and scrubbing of the floor area using floor cleaner and cleaning other areas of the rooms",
    ],
  },
  {
    img: "https://homemaster.in/upload/subservices/2BHK.jpg",
    tittle: "2 BHK",
    price: "Rs 5500 ",
    Introduction:
      "Cleaning and scrubbing the floor focusing on dirt removal from pores, stain removal and restoring shine",
    Time: "2 hrs Every time we clean, we'll provide services that include the following:very time we clean, we'll provide services that include the following: Bathrooms Cobwebs removed, dusting, floors cleaned, toilets cleaned, mirrors / chrome fixtures cleaned, shower doors cleaned, tile walls, bathtub / showers cleaned Sleeping Areas Surfaces hand wiped, floors cleaned, general dusting, cobwebs removed, doors and door frames spot cleaned Living Areas Surfaces hand wiped, cobwebs removed, doors and door frames spot cleaned, general dusting, floors cleaned Kitchen Countertops cleaned, outside of range hood cleaned, top and front of range cleaned, drip pans / glass top surfaces wiped, sinks cleaned and chrome shined, fronts of all appliances cleaned, general dusting, cobwebs removed, microwave wiped out, doors and door frames spot cleaned",
    InclusionsExclusions: [
      "Thorough detail-clean throughout your house over the course of the first two cleans",
      "First clean: Comprehensive cleaning of the apartment with special attention to the kitchen and bathrooms",
      "Second session: Continued detailed cleaning to ensure thoroughness",
      "Equipment carried: Mops, buckets, cleaning agents, dusters, garbage bags",
      "Crew size: 1 person for 1 bathroom, 2 persons for 2 to 3 bathrooms",
      "Cleaning of walls",
    ],
    Procedure: [
      "Thorough detail-clean throughout your house over the course of the first two cleans",
      "First clean: Comprehensive cleaning of the apartment with special attention to the kitchen and bathrooms",
      "Second session: Continued detailed cleaning to ensure thoroughness",
    ],
  },
  {
    img: "https://homemaster.in/upload/subservices/3BHK.jpg",
    tittle: "3 BHK",
    price: "Rs 6500 ",
    Introduction:
      "Cleaning and scrubbing the floor focusing on dirt removal from pores, stain removal and restoring shine",
    Time: "2 HRs Every time we clean, we'll provide services that include the following:Every time we clean, we'll provide services that include the following: Bathrooms Cobwebs removed, dusting, floors cleaned, toilets cleaned, mirrors / chrome fixtures cleaned, shower doors cleaned, tile walls, bathtub / showers cleaned Sleeping Areas Surfaces hand wiped, floors cleaned, general dusting, cobwebs removed, doors and door frames spot cleaned Living Areas Surfaces hand wiped, cobwebs removed, doors and door frames spot cleaned, general dusting, floors cleaned Kitchen Countertops cleaned, outside of range hood cleaned, top and front of range cleaned, drip pans / glass top surfaces wiped, sinks cleaned and chrome shined, fronts of all appliances cleaned, general dusting, cobwebs removed, microwave wiped out, doors and door frames spot cleaned",
    InclusionsExclusions: [
      "Equipment carried: Mops, buckets, cleaning agents, dusters, garbage bags",
      "Crew size: 1 person for 1 bathroom, 2 persons for 2 to 3 bathrooms",
      "Cleaning of walls",
    ],
    Procedure: [
      "Thorough detail-clean throughout your house over the course of the first two cleans",
      "First clean: Comprehensive cleaning of the apartment with special attention to the kitchen and bathrooms",
      "Second session: Continued detailed cleaning to ensure thoroughness",
      "Equipment carried: Mops, buckets, cleaning agents, dusters, garbage bags",
      "Crew size: 1 person for 1 bathroom, 2 persons for 2 to 3 bathrooms",
      "Cleaning of walls",
    ],
  },
  {
    img: "https://homemaster.in/upload/subservices/4BHK.jpg",
    tittle: "4 BHK",
    price: "On Inspection",
    Introduction:
      "Cleaning and scrubbing the floor focusing on dirt removal from pores, stain removal and restoring shine",
    Time: "2 to 6 hours, depending upon floor type and size Every time we clean, we'll provide services that include the following:Every time we clean, we'll provide services that include the following: Bathrooms Cobwebs removed, dusting, floors cleaned, toilets cleaned, mirrors / chrome fixtures cleaned, shower doors cleaned, tile walls, bathtub / showers cleaned Sleeping Areas Surfaces hand wiped, floors cleaned, general dusting, cobwebs removed, doors and door frames spot cleaned Living Areas Surfaces hand wiped, cobwebs removed, doors and door frames spot cleaned, general dusting, floors cleaned Kitchen Countertops cleaned, outside of range hood cleaned, top and front of range cleaned, drip pans / glass top surfaces wiped, sinks cleaned and chrome shined, fronts of all appliances cleaned, general dusting, cobwebs removed, microwave wiped out, doors and door frames spot cleaned",
    InclusionsExclusions: [
      "Equipment carried: Mops, buckets, cleaning agents, dusters, garbage bags",
      "Crew size: 1 person for 1 bathroom, 2 persons for 2 to 3 bathrooms",
      "Cleaning of walls",
    ],
    Procedure: [
      "Bathrooms: Cobwebs removed, dusting, floors cleaned, toilets cleaned, mirrors/chrome fixtures cleaned, shower doors cleaned, tile walls, bathtub/showers cleaned",
      "Sleeping Areas: (additional services to be specified)",
    ],
  },
  {
    img: "https://homemaster.in/upload/subservices/OFFICE.jpg",
    tittle: "Office",
    price: "On Inspection",
    Introduction:
      "Cleaning and scrubbing the floor focusing on dirt removal from pores, stain removal and restoring shine",
    Time: "2 to 6 hours, depending upon floor type and size Every time we clean, we'll provide services that include the following: Every time we clean, we'll provide services that include the following: Bathrooms Cobwebs removed, dusting, floors cleaned, toilets cleaned, mirrors / chrome fixtures cleaned, shower doors cleaned, tile walls, bathtub / showers cleaned Sleeping Areas Surfaces hand wiped, floors cleaned, general dusting, cobwebs removed, doors and door frames spot cleaned Living Areas Surfaces hand wiped, cobwebs removed, doors and door frames spot cleaned, general dusting, floors cleaned Kitchen Countertops cleaned, outside of range hood cleaned, top and front of range cleaned, drip pans / glass top surfaces wiped, sinks cleaned and chrome shined, fronts of all appliances cleaned, general dusting, cobwebs removed, microwave wiped out, doors and door frames spot cleaned",
    InclusionsExclusions: [
      "Equipment carried: Mops, buckets, cleaning agents, dusters, garbage bags",
      "Crew size: 1 person for 1 bathroom, 2 persons for 2 to 3 bathrooms",
      "Cleaning of walls",
    ],
    Procedure: [
      "Bathrooms: Cobwebs removed, dusting, floors cleaned, toilets cleaned, mirrors/chrome fixtures cleaned, shower doors cleaned, tile walls, bathtub/showers cleaned",
      "Sleeping Areas: (additional services to be specified)",
    ],
  },
  {
    img: "https://homemaster.in/upload/subservices/VILLA.jpg",
    tittle: "Villa",
    price: "On Inspection",
    Introduction:
      "Cleaning and scrubbing the floor focusing on dirt removal from pores, stain removal and restoring shine",
    Time: "2 to 6 hours, depending upon floor type and size Every time we clean, we'll provide services that include the following: Every time we clean, we'll provide services that include the following: Bathrooms Cobwebs removed, dusting, floors cleaned, toilets cleaned, mirrors / chrome fixtures cleaned, shower doors cleaned, tile walls, bathtub / showers cleaned Sleeping Areas Surfaces hand wiped, floors cleaned, general dusting, cobwebs removed, doors and door frames spot cleaned Living Areas Surfaces hand wiped, cobwebs removed, doors and door frames spot cleaned, general dusting, floors cleaned Kitchen Countertops cleaned, outside of range hood cleaned, top and front of range cleaned, drip pans / glass top surfaces wiped, sinks cleaned and chrome shined, fronts of all appliances cleaned, general dusting, cobwebs removed, microwave wiped out, doors and door frames spot cleaned",
    InclusionsExclusions: [
      "Equipment carried: Mops, buckets, cleaning agents, dusters, garbage bags",
      "Crew size: 1 person for 1 bathroom, 2 persons for 2 to 3 bathrooms",
      "Cleaning of walls",
    ],
    Procedure: [
      "Thorough detail-clean throughout your house over the course of the first two cleans",
      "First clean: Comprehensive cleaning of the apartment with special attention to the kitchen and bathrooms",
      "Second session: Cleaning the entire apartment with detail-clean services in sleeping and living areas",
      "Maintaining the detail-clean level throughout the home with deep cleaning services on a rotating basis",
    ],
  },

  {
    img: "https://5.imimg.com/data5/SELLER/Default/2023/12/367680388/OV/GZ/HU/26356278/premium-furnished-bungalow-cleaning-service.jpeg",
    tittle: "Bunglow Cleaning",
    price: "On Inspection",
    Introduction:
      "Comprehensive cleaning service for bungalows, ensuring every corner is spotless and welcoming. Special focus on floors, bathrooms, and living spaces.",
    Time: "2 to 6 hours, depending upon the size of the bungalow and level of cleaning required.",
    InclusionsExclusions: [
      "Equipment carried: Mops, buckets, cleaning agents, dusters, garbage bags",
      "Crew size: Adequate crew based on the bungalow size",
      "Cleaning of all accessible walls and surfaces",
    ],
    Procedure: [
      "Initial inspection to understand cleaning requirements",
      "Detailed cleaning of all rooms, bathrooms, and kitchens",
      "Spot cleaning and polishing to restore the original shine",
    ],
  },
  {
    img: "https://media.istockphoto.com/id/1363376686/photo/handsome-asian-man-wearing-apron-cleaning-floor-at-home-guy-washing-floor-with-mopping-stick.jpg?s=612x612&w=0&k=20&c=g2kmWxPKKi7FkfpMyFrKWL5T3iRSnxsJ2gx6hO1Eha8=",
    tittle: "Raw House Cleaning",
    price: "On Inspection",
    Introduction:
      "Professional cleaning of raw houses to prepare them for move-in. Focus on removing dust, debris, and stains.",
    Time: "2 to 6 hours, depending on the house's condition and size.",
    InclusionsExclusions: [
      "Equipment carried: Industrial-grade cleaning tools",
      "Crew size: Adequate crew for efficient cleaning",
      "Debris and rubble removal not included",
    ],
    Procedure: [
      "Removing dust and debris from all rooms",
      "Scrubbing and polishing floors and surfaces",
      "Final inspection for quality assurance",
    ],
  },
  {
    img: "https://media.istockphoto.com/id/1372306674/photo/man-cleaning-the-terrace-tiles-of-the-apartment.jpg?s=612x612&w=0&k=20&c=2XvsouVZfMi1KcZn42ZzWp-KLPrxmyBx7Xp7wx-CHHg=",
    tittle: "Farm House Cleaning",
    price: "On Inspection",
    Introduction:
      "Deep cleaning services tailored for farmhouses, ensuring clean and fresh spaces amidst the countryside.",
    Time: "2 to 8 hours, depending on the farmhouse's size and requirements.",
    InclusionsExclusions: [
      "Equipment carried: Mops, brushes, eco-friendly cleaning agents",
      "Crew size: Adjusted to the number of rooms",
      "Cleaning of outdoor spaces upon request",
    ],
    Procedure: [
      "Thorough dusting and cleaning of indoor areas",
      "Deep cleaning of kitchens, bathrooms, and living spaces",
      "Optional cleaning of outdoor patios and verandas",
    ],
  },
  {
    img: "https://www.shutterstock.com/image-photo/cleaner-female-homeowner-tidying-home-260nw-2479511663.jpg",
    tittle: "Project Cleaning",
    price: "On Inspection",
    Introduction:
      "Post-construction cleaning for residential or commercial projects, ensuring debris-free and ready-to-use spaces.",
    Time: "4 to 12 hours, depending on the project size and complexity.",
    InclusionsExclusions: [
      "Equipment carried: Heavy-duty cleaning tools and vacuum cleaners",
      "Crew size: Assigned based on project scale",
      "Debris removal outside the premises not included",
    ],
    Procedure: [
      "Dust and debris removal from floors and surfaces",
      "Window and fixture cleaning",
      "Polishing and deep cleaning of essential areas",
    ],
  },
  {
    img: "https://media.istockphoto.com/id/518333250/photo/steam-cleaning-the-office-carpet.jpg?s=612x612&w=0&k=20&c=UlIKHazn11ghDXwUmMlVlQepkbtk_G_WybVIhv_78HI=",
    tittle: "Office Carpet Shampooing",
    price: "On Inspection",
    Introduction:
      "Specialized carpet cleaning service for offices, using high-quality shampoos to remove stains and odors.",
    Time: "1 to 4 hours, depending on carpet area and level of cleaning.",
    InclusionsExclusions: [
      "Equipment carried: Carpet shampooers, brushes, and drying fans",
      "Crew size: Adjusted to carpet size and office space",
      "Furniture moving not included unless specified",
    ],
    Procedure: [
      "Pre-cleaning inspection and vacuuming",
      "Application of cleaning agents and shampooing",
      "Post-cleaning drying and quality check",
    ],
  },
  {
    img: "https://www.leofabasia.com/images/services/facade-cleaning.jpg",
    tittle: "Facade Cleaning",
    price: "On Inspection",
    Introduction:
      "Exterior cleaning of building facades, ensuring a clean and polished look for your property.",
    Time: "2 to 6 hours, depending on the facade's size and condition.",
    InclusionsExclusions: [
      "Equipment carried: Pressure washers, cleaning agents, and safety gear",
      "Crew size: Trained professionals with proper safety equipment",
      "Cleaning restricted to accessible areas",
    ],
    Procedure: [
      "Inspection of facade and cleaning plan setup",
      "Pressure washing and scrubbing of exterior surfaces",
      "Final polish to enhance appearance",
    ],
  },
  {
    img: "https://t4.ftcdn.net/jpg/03/20/28/73/360_F_320287361_Cq6lb6COc1Wwd7gF1jUoGCz3Waq5mO8o.jpg",
    tittle: "Bathroom Cleaning",
    price: "Rs 450",
    Introduction:
      "Professional bathroom cleaning service to remove stains, mold, and ensure a hygienic space.",
    Time: "1 to 3 hours, depending on bathroom size and number.",
    InclusionsExclusions: [
      "Equipment carried: Scrubbers, cleaning agents, and disinfectants",
      "Crew size: 1 cleaner per bathroom",
      "Excludes repair or plumbing work",
    ],
    Procedure: [
      "Deep cleaning of tiles, grout, and fixtures",
      "Sanitization of sinks, toilets, and showers",
      "Polishing of mirrors and glass surfaces",
    ],
  },
  {
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSoCNvSCgwH_PnO1Kbsz1GYnKWYYdrvxJtrnA&s",
    tittle: "Kitchen Cleaning",
    price: "Rs 1200 ",
    Introduction:
      "Professional bathroom cleaning service to remove stains, mold, and ensure a hygienic space.",
    Time: "1 to 3 hours, depending on bathroom size and number.",
    InclusionsExclusions: [
      "Equipment carried: Scrubbers, cleaning agents, and disinfectants",
      "Crew size: 1 cleaner per bathroom",
      "Excludes repair or plumbing work",
    ],
    Procedure: [
      "Deep cleaning of tiles, grout, and fixtures",
      "Sanitization of sinks, toilets, and showers",
      "Polishing of mirrors and glass surfaces",
    ],
  },
  {
    img: "https://img.freepik.com/free-photo/full-shot-man-vacuuming-couch_23-2149482278.jpg?semt=ais_hybrid",
    tittle: "Sofa Cleaning",
    price: "Rs 200 Per Seat",
    Introduction:
      "Professional bathroom cleaning service to remove stains, mold, and ensure a hygienic space.",
    Time: "1 to 3 hours, depending on bathroom size and number.",
    InclusionsExclusions: [
      "Equipment carried: Scrubbers, cleaning agents, and disinfectants",
      "Crew size: 1 cleaner per bathroom",
      "Excludes repair or plumbing work",
    ],
    Procedure: [
      "Deep cleaning of tiles, grout, and fixtures",
      "Sanitization of sinks, toilets, and showers",
      "Polishing of mirrors and glass surfaces",
    ],
  },
  {
    img: "https://t3.ftcdn.net/jpg/02/18/97/04/360_F_218970417_HqekNsoEJHkbN14V89RKagpY4sY2V6dd.jpg",
    tittle: "Home  Carpet Cleaning",
    price: "On inspection",
    Introduction:
      "Professional bathroom cleaning service to remove stains, mold, and ensure a hygienic space.",
    Time: "1 to 3 hours, depending on bathroom size and number.",
    InclusionsExclusions: [
      "Equipment carried: Scrubbers, cleaning agents, and disinfectants",
      "Crew size: 1 cleaner per bathroom",
      "Excludes repair or plumbing work",
    ],
    Procedure: [
      "Deep cleaning of tiles, grout, and fixtures",
      "Sanitization of sinks, toilets, and showers",
      "Polishing of mirrors and glass surfaces",
    ],
  },
  {
    img: "https://media.istockphoto.com/id/1150282133/photo/cleaning-window-pane-with-detergent.jpg?s=612x612&w=0&k=20&c=BkzVas8qZ_VLxLDWpDAPbVSDRXgRFMxbI3_NHcamUfk=",
    tittle: "Window  Cleaning",
    price: "Rs 1800 ",
    Introduction:
      "Professional bathroom cleaning service to remove stains, mold, and ensure a hygienic space.",
    Time: "1 to 3 hours, depending on bathroom size and number.",
    InclusionsExclusions: [
      "Equipment carried: Scrubbers, cleaning agents, and disinfectants",
      "Crew size: 1 cleaner per bathroom",
      "Excludes repair or plumbing work",
    ],
    Procedure: [
      "Deep cleaning of tiles, grout, and fixtures",
      "Sanitization of sinks, toilets, and showers",
      "Polishing of mirrors and glass surfaces",
    ],
  },
  {
    img: "https://homemaster.in/upload/servicedetails/Water_Tank1.jpg",
    tittle: "Water Tank Cleaning Service",
    price: "On Inspection",
    Introduction:
      "Our Water Tank Service ensures that your water storage system is clean, safe, and well-maintained, providing you with fresh and hygienic water for daily use.",
    Time: "2 to 6 hours, depending upon floor type and size Every time we clean, we'll provide services that include the following Our Water Tank Service offers a complete cleaning and maintenance solution for your water storage system. By removing contaminants and disinfecting the tank, we ensure that your water remains fresh and safe for consumption. Regular cleaning helps extend the life of your tank and improves the quality of your water.",
    InclusionsExclusions: [
      "Full inspection and cleaning of the water tank",
      "Removal of dirt, sludge, and algae buildup",
      "Disinfection of the tank to prevent contamination",
      "Checking and cleaning of inlet and outlet pipes",
      "Repair or replacement of damaged tank components (unless specified)",
      "Installation of new water tanks or plumbing systems",
      "Cleaning of external areas outside the tank",
    ],
    Procedure: [
      "Inspection of the water tank to assess its condition and cleanliness",
      "Draining of the tank and thorough cleaning of the interior surfaces",
      "Removal of sludge, algae, and other contaminants",
      "Sanitizing the tank with safe, non-toxic disinfectants",
      "Refill the tank and ensure proper functioning of inlet and outlet pipes",
    ],
  },
];

export default data4;
